...
using System.Xml.Serialization; // To use Xml
using System.Text.Json; // By writting FastJson, we use the System.Text.Json.JsonSerializer
...
partial class MainProgram
{
    static async Task Main(string[] args)
    {
        ...
        string path = Combine(Environment.CurrentDirectory, "people.xml");
        ...
        // Create a file to write to.
        string jsonPath = Combine(Environment.CurrentDirectory, "people.json");
        ...
        await using (FileStream jsonLoad = File.Open(jsonPath, FileMode.Open))
        {
            ...
            List<Person>? loeadedPeople = await JsonSerializer.DeserializeAsync(utf8Json: jsonLoad, returnType: typeof(List<Person>)) as List<Person>;

            if(loeadedPeople is not null)
            {
                foreach(Person p in loeadedPeople)
                {
                    Console.WriteLine($"{p.LastName} has {p.Children?.Count??0}.");
                }    
            }
